...
**Last updated:** 2026-06-12(PR-23.5a〔CF4〕✅ merged `70f93d9` #271 真打 PASS〔Sprint 23 + 49KB 附件實證〕;PR-23.5b §1 靜態核心翻新 shipping;Sprint 23 進行中)
...
- **當前 PR:** PR-23.5b snapshot §1 靜態核心翻新(shipping `0d25d0c`)— 微型純文字 + 版本動態注入(來源 = `_resolve_progress_path` 選中檔名,同一來源,v4.0 開檔自動跟上):「NEXW vX.Y 建造中」+ 不變事實 4 行;**文案禁止寫死 Sprint / PR 編號**(Sprint 5 時代舊文案〔v2.0 開發中 / v1.0 FROZEN / PR-5.x〕曾腐爛一整個大版本,測試鎖無腐爛字)。23.6 接手驗收前置。
- **Next action:** PR-23.5b merge 後 TEEMO:rebuild `central-bot` + `discord-bot` 只 restart → 真打:Discord `/snapshot` 一發,逐字核 **§1 首行 =「NEXW v3.0 建造中」+ 無任何 PR-5.x / v1.0 FROZEN 殘句**(~$0.2,同時當 23.6 接手包預熱驗收)→ 23.6 前真實新 session 接手最終驗收(先過河再拆橋)→ 23.6 鏡射退役 + PR-23.3b(A2 多輪)
...
- **(shipping)PR-23.5b** snapshot §1 靜態核心翻新(`0d25d0c`)— 微型純文字(23.6 接手驗收前置;起因:CF4 真打截圖 §1 仍是 Sprint 5 時代文案,23.6 後此檔是新 session 第一眼世界觀)。`_collect_docs_reality` 加 `progress_version`(從 `_resolve_progress_path` 選中檔名取 — **與路徑解析同一來源**,v4.0 開檔自動跟上)→ `_compose_static_core` 翻新 4 行:「NEXW vX.Y 建造中(當前 Sprint / HEAD 見上方欄位)/ TEEMO 一人 PM + chat 規劃 Code 實作 / 鐵律〔Secret 不貼 chat・git ground truth・Sprint 編號 = 規格書・Swagger〕/ 規格書 docs/automation/vX.Y.md + 前代已封版 + 雙前台過渡中」。**禁止寫死 Sprint / PR 編號落實:** 測試鎖 `PR-` / `FROZEN` / `Sprint <數字>` 不得出現於 §1 + 版本跟最新檔(fixture 加 v3.0 → 自動 v3.0)。引擎其餘 0 改(解析/regex/路徑/LLM 段)。forbidden 0 改:central 其餘 / bot / dbot / compose / Dockerfile / requirements。3 層綠(central 1104〔+2〕/ dbot 91 / bot 379 + ruff);真實樹 sanity:§1 渲染 =「NEXW v3.0 建造中 …」✅。⚠️ **runtime-wired → merge 後 rebuild `central-bot` + `discord-bot` 只 restart** → 真打逐字核 §1 首行 + 無殘句(兼 23.6 預熱驗收)。
...
**Last updated**: 2026-06-12(PR-23.5a〔CF4〕✅ merged `70f93d9` #271 真打 PASS;PR-23.5b §1 靜態核心翻新 shipping;Sprint 23 進行中)
**當前 Sprint**: v3.0 階段 — Sprint 23 進行中(Discord bot 地基 + adapter);v2.0 已封版(2026-06-09),live 進度見 v3.0-progress.md
**當前 PR**: PR-23.5b snapshot §1 靜態核心翻新(shipping `0d25d0c`,23.6 接手驗收前置)
**Next action**: PR-23.5b merge 後 rebuild `central-bot` + `discord-bot` 只 restart → Discord `/snapshot` 真打逐字核:**§1 首行 =「NEXW v3.0 建造中」+ 無 PR-5.x / v1.0 FROZEN 殘句**(兼 23.6 預熱驗收)→ 23.6 前真實新 session 接手最終驗收 → 23.6 鏡射退役 + PR-23.3b(A2 多輪)。PR-D12 V1-V5 驗收(轉 private)依既排程推進。**R-001 已廢止 30 天設計凍結(2026-06-11 提前解凍,原至 2026-07-09)→ v3.0 即日動工**,建造期新需求進 v4.0 候選池。**v2.0 封版後續追(非 blocker,見 `v2.0-acceptance-report.md §3`):** KI-PR-5.4-B([Merge] 按鈕,v3.0 Sprint 26 DM4 順帶解)/ H6 Gate2 待 PAT `Checks:read` / H8b Flutter pipeline 待首客戶 / C1 briefing richer 待 L2 / koalo typesense healthcheck / registry housekeeping 2 項 / KI-update-detached-head / effort 值 / ~~snapshot regex(CF4)~~ **CF4 ✅ 銷項(PR-23.5a)**。**v4.0 候選池:** 見 v3.0.md §5(自動商業 pipeline / AI 即時語音 / Client 頻道 …)。
