...
**Last updated:** 2026-06-12(PR-23.5b ✅ merged `c85d1dd` #272 真打 PASS〔§1 翻新逐字驗證〕;PR-23.5c §6 接手 SOP 翻新 shipping;Sprint 23 進行中)
...
- **當前 PR:** PR-23.5c snapshot §6 接手 SOP 翻新(shipping `94e74f5`)— 微型純文字,23.5b 同模式;接手包 23.6-ready 最後一塊。起因(23.5b 真打附件全文核):§6 仍寫「Project: NEXW v2.0」+「web_fetch 5 個 docs URL」— 後者 D-12 後不可行(repo private,raw 403);錯的指南比沒有指南糟。新 5 步:上傳 .md → **三欄對齊回報(當前 Sprint / 當前 PR / main HEAD)** → private 紀律 → 照常 PR 軌道。抗腐蝕鎖:§6 無 v2.0 / 無 web_fetch / 含 private / 不引用鏡射(23.6 退役不烤進)。
- **Next action:** PR-23.5c merge 後 TEEMO:rebuild `central-bot` + `discord-bot` 只 restart → 真打:Discord `/snapshot` 一發核 §6 新文案(~$0.2;**這發同時就是 23.6 接手驗收要用的那份檔,一魚兩吃**)→ 真實新 session 接手最終驗收 → 23.6 鏡射退役(close 待辦含 current-state §8 封存清掃)+ PR-23.3b(A2 多輪)
...
- **(shipping)PR-23.5c** snapshot §6 接手 SOP 翻新(`94e74f5`)— 微型純文字(0 邏輯,f-string 內 5 行;23.6-ready 最後一塊)。新 SOP:① fresh chat(NEXW Project,無版本號)② 上傳 .md(或貼 §1-§4)③ **接手第一步 = 三欄對齊回報**(當前 Sprint / 當前 PR / main HEAD;不對齊 → server snapshot 拍板)④ private 紀律(規格經 Claude Code / server 讀;不可網頁直抓 raw URL〔403〕— spec 提案原句含 web_fetch 字面,為守鎖改寫同義)⑤ 照常 PR 軌道(§0-§3 紀律)。抗腐蝕鎖測試(23.5b no_rotting_words 延伸):§6 slice 斷言無 `v2.0` / 無 `web_fetch` / 含 `private` / 無鏡射引用 / 含三欄句。forbidden 0 改:§1 文案 / 解析 / regex / LLM 段 / central 其餘 / bot / dbot / compose / Dockerfile / requirements。3 層綠(central 1105〔+1〕/ dbot 91 / bot 379 + ruff)。⚠️ **runtime-wired → merge 後 rebuild `central-bot` + `discord-bot` 只 restart** → 真打 `/snapshot` 核 §6(兼 23.6 接手驗收檔)。
- **(已 merged)PR-23.5b** snapshot §1 靜態核心翻新 ✅ merged `c85d1dd`(#272)**真打 PASS(§1 翻新逐字驗證:「NEXW v3.0 建造中」+ 無殘句)** — 版本動態注入(`_resolve_progress_path` 同一來源)+ 抗腐蝕文案 4 行;測試鎖無腐爛字(PR- / FROZEN / Sprint 數字)。
- **(已 merged)PR-23.5a(CF4)** snapshot 引擎改讀 active progress + sprint regex 韌性放寬 ✅ merged `70f93d9`(#271)**真打 PASS(「當前 Sprint」= Sprint 23 + 49KB 附件實證;CF4 自封版後續追清單銷項確認)** — central 小刀(比照 23.4a 模式,服役 plane 不跳步)。`_resolve_progress_path`:glob `v*-progress.md` → `re.fullmatch` 守門 → `(major, minor)` **數值** tuple 排序取最新(v10>v9 鎖測,字串排序翻車是經典雷;封存 v2.0 天然落選;空集 FileNotFoundError 走既有 class-name-only 路徑)。`_parse_current_sprint` regex 放寬:容忍冒號 bold 內/外 + 全/半形(同一份 v3.0-progress 實測 L26/L217 兩種寫法並存 — 漂移是現在進行式非假想;純放寬不收窄)。`:563` fallback 文案改版本中立;docstring 同步。Recon 意外發現存檔:**Sprint 0 根因 100% 是讀錯檔** — 舊 regex 對 v3.0 canonical 行本來就 match,v2.0 封版檔該行寫「無」才 fallback 0。切段/校正紀錄抽取 0 改(v3.0 `## 當前狀態` 實測切得動,1,062 chars)。**隱藏收益(CF4 紅利):§3 段 80,093 → 1,062 chars,LLM prompt ~115K → ~5K,`/snapshot` 每打成本大降。** 測試 +9(多版本取最新 / v10>v9 數值排序 / 封存不選中 / 單檔仍選中 / 空集 raise / fullmatch 擋非版本名 / 冒號 bold 外 / 全形冒號 / 封版「無」行回歸鎖)。真實 docs 樹 sanity:resolved=v3.0-progress.md,sprint=23,§3=1,062 chars ✅。forbidden 0 改:central 其餘 / bot / dbot / compose / Dockerfile / requirements。3 層綠(central 1102〔含 +9〕/ dbot 91 / bot 379 + ruff)。⚠️ **runtime-wired → merge 後 rebuild `central-bot`;`discord-bot` 只 restart(活讀 /repo)** → 真打逐字核:「當前 Sprint」= Sprint 23 + §3 段來自 v3.0-progress。
- **(已 merged)PR-23.5** DB5-前段 `/snapshot` 接手包 Discord 化 ✅ merged `43e549d`(#270)**真打 PASS(Discord 全鏈成功〔含 git push `8a91b51`〕+ TG graceful degrade 實證〔`a41be87`〕)** — 新 `dbot/snapshot_entry.py`(discord-free 核心,比照 idea_entry 可單測):project 邊界三條 parity TG(預設 `nexw-deploy-bot`;`all` 拒;其他拒,拒答文字逐字同 TG)→ `SnapshotGenerator().generate(push_to_git=True)`(central 引擎 0 改)→ 摘要 + `.md` 附件三件套(filename = 引擎 file_path basename,與 TG 同名;caption 標 push 成功/失敗);lifecycle 歸 caller try/finally `aclose`;例外只露 `type(exc).__name__`(secret 紀律)。`commands.py` 掛第二支 nested slash:admin gate(`dcfg.is_admin`,deny ephemeral)→ **`defer(thinking=True)`**(R3 實證 LLM `claude-sonnet` 一次 ~10-15s,超 interaction 3 秒時限)→ followup 摘要 chunk(引擎 cap ≤3000 → ≤2 則)+ **完整接手包恆附件**(R3 實測 ~119K chars,訊息 chunk 不可行;裁示否決門檻方案)。main.py 0 改(guild sync 自動帶 2 支)。runtime 依賴逐項核(L-runtime-binary):git binary ✅(23.3a-hotfix)/ `/repo:ro` ✅ / xxnexw PAT 檔 `/data/secrets/github/api-token-xxnexw.txt` ✅(dbot `/data` 掛載已達)/ LiteLLM proxy ✅(DB3 真打實證);**0 新 pip 依賴**(閉包唯一外部 pip = httpx 已備)。命名 `snapshot_entry` 不撞 central(guard 動態驗)。forbidden 0 改:central 全部 / bot / compose / Dockerfile / requirements / dbot 既有模組除 commands.py。3 層綠(dbot 91〔含 11 新〕/ central 1093 / bot 379 + ruff + py_compile)。⚠️ **runtime-wired → merge 後 rebuild discord-bot + 真打**(Discord vs TG `/snapshot` 同日對照逐字核;23.6 前真實新 session 接手最終驗收)。
- **(已 merged)PR-23.4** DB4 agent 結果鏡射 Discord ✅ merged `e376109`(#269)**真打三驗 PASS(baseline 不洗版 / exec+error 雙流向 / 重啟不重貼)** — 新 `dbot/agent_mirror.py`(discord-free 核心,比照 idea_entry 可單測):雙 glob `*.result.delivered.json`(正常件)+ `*.result.json`(central 逾時遲到孤兒)+ id dedupe(delivered 優先,只貼一次)→ status 分流 `agent-exec-log`(done:`✅ agent <id> 完成|<started→finished>` + output)/ `agent-error-log`(error:`❌ agent <id> 失敗` + error);正文 >1500 截斷 + `(截斷,完整:/srv/agent-jobs/<檔名>)`;送出走 adapter chunk(2000)。狀態檔 `/data/dbot/agent-mirror.json` `{"seen":{id:ts}}`(central `atomic_io` atomic 寫 + 首次 mkdir parents;首跑 baseline 全標 seen 不貼;重啟不重貼)。耐髒:壞 JSON / 讀檔瞬間 FileNotFoundError(central 正 rename)→ WARNING skip 不標 seen 下輪再試;單檔不影響同輪其他檔。接線:main.py `setup_hook` 掛背景 task(**非 on_ready** — 重連重發會疊 task);單輪全包 try/except 不殺迴圈;頻道解析比照 boss-inbox(任一缺 → 該流向停用,另一流向照跑)。compose 唯一改動:dbot 補掛 `- /srv/agent-jobs:/jobs:ro` 一行(唯讀 kernel 強制 + 模組零寫入雙保險)。poll `DBOT_AGENT_MIRROR_POLL_S` 預設 10s floor 5s。不撞保留字(guard 動態驗,central 無 agent_mirror)。forbidden 0 改:central 全部 / bot 全部 / Dockerfile / requirements / compose 其餘 / dbot 既有模組除 main.py 接線。3 層綠(dbot 79〔含 18 新〕/ central 1093 / bot 379 pass + ruff + py_compile + compose YAML)。⚠️ **runtime-wired → merge 後 rebuild discord-bot + `up -d` recreate(吃新掛載)+ Discord 真打**(baseline 不洗版 / exec+error 雙流向 / 重啟不重貼)。
- **(已 merged)PR-23.4a** central cleanup:unlink → rename `.delivered` 終態檔 ✅ merged `6745868`(#268)**真打 PASS(TG deliver 0 變 + `.delivered` 終態檔實證)** — v3.0 第一刀進 central,範圍刻意最小:`central/agent_jobs.py` `cleanup()` 由 `path.unlink()` 改 `path.replace(<id>.result.delivered.json)`(stdlib 原子覆蓋;FileNotFoundError pass / OSError warning 的 best-effort 語意不變)+ docstring 同步(終態檔供 dbot 鏡射,堆積治理屬 O3 清理 KI)。consumer 清點(§1 Recon):runner 寫 ×2 / read_result 讀 / cleanup 刪,無第四消費者;`.delivered.json` 不被任何既有 glob 撈到(runner 只 filter `.job.json`,central 全精確檔名)。call site(`handlers/agent.py:136`)在 `_deliver` 之後 → TG 行為 0 變、0 改。測試:`TestCleanup` 改 3 測(rename + 內容不變 / 缺檔 noop / 已存在覆蓋)。forbidden 0 改:central 其餘 / bot / dbot / compose / Dockerfile / requirements。3 層綠(central 1093 / bot 379 / dbot 61 pass)。⚠️ **runtime-wired → merge 後 rebuild `central-bot` + L18 smoke + `/agent` 真打**(推完 TG 後 `/jobs` 見 `.delivered` 檔)。
...
- **(候選 lesson,PR-23.3-hotfix)L-dbot 活讀 central ⇒ 逐項核 runtime 依賴:** dbot 容器活讀 /repo/central 源碼,但 image 只裝 `dbot/requirements.txt` 的 pip + 自己 Dockerfile 的 binary ——**central 鏈的 runtime 依賴(binary + pip)必須逐項核進 dbot image,Recon 只核 env 不夠**。git binary(PR-23.3a-hotfix)、httpx(PR-23.3-hotfix)兩案同族:`import` 鏈走到 central 模組就會踩到 central 的 pip 依賴。fail-safe 設計會把 ImportError 吞成降級(無聲)→ 加開機預檢把同族問題翻成「第一眼可見」。**新接 central 模組前先 walk 它的 import 樹,把非 stdlib 依賴逐個對照 dbot image 是否帶齊。**
...
**Last updated**: 2026-06-12(PR-23.5b ✅ merged `c85d1dd` #272 真打 PASS;PR-23.5c §6 接手 SOP 翻新 shipping;Sprint 23 進行中)
**當前 Sprint**: v3.0 階段 — Sprint 23 進行中(Discord bot 地基 + adapter);v2.0 已封版(2026-06-09),live 進度見 v3.0-progress.md
**當前 PR**: PR-23.5c snapshot §6 接手 SOP 翻新(shipping `94e74f5`,23.6-ready 最後一塊)
**Next action**: PR-23.5c merge 後 rebuild `central-bot` + `discord-bot` 只 restart → `/snapshot` 真打核 §6 新文案(兼 23.6 接手驗收檔,一魚兩吃)→ 真實新 session 接手最終驗收 → 23.6 鏡射退役(close 待辦:current-state §8 封存清掃〔Resolved 條目移 close report,§4 KI 段 34K → live-only〕— 0-code docs 工)+ PR-23.3b(A2 多輪)。PR-D12 V1-V5 驗收(轉 private)依既排程推進。**R-001 已廢止 30 天設計凍結(2026-06-11 提前解凍,原至 2026-07-09)→ v3.0 即日動工**,建造期新需求進 v4.0 候選池。**v2.0 封版後續追(非 blocker,見 `v2.0-acceptance-report.md §3`):** KI-PR-5.4-B([Merge] 按鈕,v3.0 Sprint 26 DM4 順帶解)/ H6 Gate2 待 PAT `Checks:read` / H8b Flutter pipeline 待首客戶 / C1 briefing richer 待 L2 / koalo typesense healthcheck / registry housekeeping 2 項 / KI-update-detached-head / effort 值 / ~~snapshot regex(CF4)~~ **CF4 ✅ 銷項(PR-23.5a)**。**v4.0 候選池:** 見 v3.0.md §5(自動商業 pipeline / AI 即時語音 / Client 頻道 …)。
